...
wifi_data8 <- wifi_data7[,(names(wifi_data7 %>%
                                   select(starts_with("WAP"))) %in% 
                             names(wifi_validation3 %>%
...
#tá faltando long, lat e floor. vou tirar da outra tabela e dar um bind
wifi_data7 %>% select("LONGITUDE", "LATITUDE", "FLOOR") -> wifi_temp

#unir com a outra tabela
wifi_data9 <- cbind(wifi_data8, wifi_temp)

#vou aplicar o zeroVariance again, porque alguns waps passaram 
#a ter zero var depois de alguma linhas eliminadas
wifi_var0_3 <- nearZeroVar(wifi_data9, saveMetrics = TRUE)

x <- 0

cols_ok_3 <- c(rownames(wifi_var0_3[wifi_var0_3$freqRatio > x,]))

wifi_data10 <- as.data.frame(wifi_data9 [,cols_ok_3])

#vou eliminar o id
wifi_data11 <- wifi_data10[,-c(1,2)]

#vou colcoar o wap001
wifi_data8 %>% select("WAP001") -> wap1_col

#juntar wap001
wifi_data12 <- cbind(wifi_data11, wap1_col)

#colocar wap001 no começo
wifi_data12 <- wifi_data12[, c(319, 1:318)]
